...
void SHT2x_SoftReset(void){
	uint8_t cmd = 0xfe;
	HAL_I2C_Master_Transmit(_sht2x_ui2c, 0x40 << 1, &cmd, 1, 1000);
}
...
uint8_t SHT2x_ReadUserReg(void) {
	...
	uint8_t cmd = 0xe7;
	HAL_I2C_Master_Transmit(_sht2x_ui2c, 0x40 << 1, &cmd, 1, 1000);
	HAL_I2C_Master_Receive(_sht2x_ui2c, 0x40 << 1, &val, 1, 1000);
	return val;
}
...
uint16_t SHT2x_GetRaw(uint8_t cmd) {
	uint8_t val[3] = { 0 };
	HAL_I2C_Master_Transmit(_sht2x_ui2c, 0x40 << 1, &cmd, 1, 1000);
	HAL_I2C_Master_Receive(_sht2x_ui2c, 0x40 << 1, val, 3, 1000);
...
void SHT2x_SetResolution(SHT2x_Resolution res) {
	...
	val = (val & 0x7e) | res;
	uint8_t temp[2] = { 0xe6, val };
	HAL_I2C_Master_Transmit(_sht2x_ui2c, 0x40 << 1, temp, 2, 1000);
}
